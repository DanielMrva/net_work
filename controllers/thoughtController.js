...
module.exports = {
    ...
    getSingleThought(req, res) {
        Thought.findOne({thoughtId: req.params.thoughtId})
            .select('-__v')
            .then((thought) => !thought ? res.status(404).json({message: 'No thought by that id'}):res.json(thought))
            .catch((err) => res.status(500).json(err));
    },
    ...
    deleteThought(req, res) {
        Thought.findOneAndDelete({thoughtId: req.params.thoughId})
        .then((thought) => 
            !thought
                ? res.status(404).json({message: 'No thought found by that id'})
